...
> 本机实际输出目录为 `runs/train/voc_baseline2/`（因重复运行自动递增编号）。训练在第 48 epoch 因 DataLoader 读图异常中断，但 `best.pt` 已保存。
...
> 本机实际输出目录为 `runs/train/voc_finetuned2/`。中途中断后可用以下命令续训：

```bash
python train.py \
  --resume runs/train/voc_finetuned2/weights/last.pt \
  --data data/VOC.yaml \
  --epochs 50 \
  --batch-size 32 \
  --img 640 \
...
plot_results('runs/train/voc_baseline2/results.csv'); \
plot_results('runs/train/voc_finetuned2/results.csv')"
```
...
| 实验 | 最优权重 | 最后权重 |
|------|----------|----------|
| Baseline | `yolov5/runs/train/voc_baseline2/weights/best.pt` | `.../last.pt` |
| 调优微调 | `yolov5/runs/train/voc_finetuned2/weights/best.pt` | `.../last.pt` |
...
|------|----------|-----------|
| `results.csv` | `runs/train/voc_baseline2/results.csv` | `runs/train/voc_finetuned2/results.csv` |
| `results.png` | `runs/train/voc_baseline2/results.png` | `runs/train/voc_finetuned2/results.png` |
| `hyp.yaml` | `runs/train/voc_baseline2/hyp.yaml` | `runs/train/voc_finetuned2/hyp.yaml` |
| `opt.yaml` | `runs/train/voc_baseline2/opt.yaml` | `runs/train/voc_finetuned2/opt.yaml` |
...
# Baseline 微调模型
python val.py \
  --weights runs/train/voc_baseline2/weights/best.pt \
  --data data/VOC.yaml \
  --img 640 \
  --verbose \
  --device 0 \
...
# 调优微调模型
python val.py \
  --weights runs/train/voc_finetuned2/weights/best.pt \
  --data data/VOC.yaml \
  --img 640 \
  --verbose \
  --device 0 \
...
└── yolov5/
    ...
    └── runs/
        ├── train/
        │   ├── voc_baseline2/          # Baseline 训练产物
        │   │   ├── weights/best.pt
        │   │   ├── results.csv
        │   │   └── results.png
        │   └── voc_finetuned2/         # 调优训练产物（50 epoch）
