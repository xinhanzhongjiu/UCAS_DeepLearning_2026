...
**实验环境：** Conda 环境 `ocr`，Python 3.10，PyTorch，NVIDIA A800 GPU。
...
**最佳开发集 BLEU-4：20.28**（第 10 个 epoch）。测试集翻译结果输出至 `data/pred.test.en.txt`，最优模型保存为 `checkpoints/best_transformer_mt.pt`。
...
- 引入 **BPE/SentencePiece 子词切分**，降低 OOV 率，扩大有效词表覆盖。
- 使用 **Beam Search** 替代贪心解码，通常可提升 1–3 个 BLEU 点。
- 增大模型规模（层数、隐藏维度）或采用 **预训练模型**（如 mBART、mT5）进行微调。
- 增加训练轮数并配合 **早停（Early Stopping）** 策略，避免过拟合。
- 引入 **反向翻译（Back-Translation）** 等数据增强方法扩充训练语料。
